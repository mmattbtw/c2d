...
const messages = {
    ...
    "string.added": "**{user}** added string `{string_identifier}` to `{project}` ✅",
    "string.updated": "**{user}** updated string `{string_identifier}` in `{project}` ➡️",
    "string.deleted": "**{user}** deleted string `{string_identifier}` from `{project}` 🟥",
...
function getMessage(key, event) {
    if (event.language !== undefined)
        event.language = langs[event.language];
